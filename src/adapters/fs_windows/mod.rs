...
impl FsPort for FsWindowsAdapter {
    ...
    async fn create_output_file(&self, file_path: &str) -> Result<(), DomainError> {
        ...
        if let Some(parent) = Path::new(file_path).parent() {
            self.create_directory(parent.to_string_lossy().as_ref())
                .await?;
        }
    ...
    async fn validate_path(&self, file_path: &str) -> Result<bool, DomainError> {
        ...
        if let Some(filename) = path.file_name() {
            if let Some(filename_str) = filename.to_str() {
                ...
                if filename_upper.starts_with("CON.")
                    || filename_upper.starts_with("PRN.")
                    || filename_upper.starts_with("AUX.")
                    || filename_upper.starts_with("NUL.")
                {
                    return Ok(false);
                }
            }
