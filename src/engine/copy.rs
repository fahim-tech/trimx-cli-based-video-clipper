...
impl StreamCopyClipper {
    ...
    pub fn clip(&self, config: EngineConfig) -> TrimXResult<ClippingProgress> {
        ...
        match self.execute_stream_copy(&config) {
            Ok(progress) => {
                ...
                info!("Stream copy clipping completed successfully in {:.2}s", elapsed.as_secs_f64());
                Ok(progress)
            }
            Err(e) => {
    ...
    fn copy_stream_info(
        ...
        for input_stream in input_ctx.streams() {
            ...
            if self.debug {
                debug!("Copied stream {}: codec={:?}, time_base={:?}", 
                       input_stream.index(),
    ...
    fn copy_packets(
        ...
        for (input_stream, mut packet) in input_ctx.packets() {
            ...
            let packet_time = if let Some(pts) = packet.pts() {
                ...
                (pts as f64 * stream_tb.numerator() as f64 / stream_tb.denominator() as f64 * ffmpeg_next::ffi::AV_TIME_BASE as f64) as i64
            } else {
            ...
            let output_stream = &output_ctx.stream(output_stream_index)
                .ok_or_else(|| TrimXError::ClippingError {
        ...
        info!("Copied {} packets ({} bytes) in total", packets_copied, total_bytes);
    ...
    pub fn estimate_output_size(&self, config: &EngineConfig) -> TrimXResult<u64> {
        ...
        let duration_ratio = (config.end_time - config.start_time) / self.get_input_duration(config)?;
