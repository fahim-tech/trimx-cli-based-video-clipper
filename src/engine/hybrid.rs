...
use crate::engine::{
    ClippingPhase, ClippingProgress, EngineConfig, ProgressTracker, ReencodeClipper,
    StreamCopyClipper,
};
use crate::error::{TrimXError, TrimXResult};
use crate::planner::keyframe_analyzer::{GOPAnalysis, KeyframeAnalyzer};
use crate::planner::{ClippingStrategy, CutPlan};
use std::sync::Arc;
...
impl Default for HybridClipper {
    fn default() -> Self {
        Self::new()
    }
}
...
impl HybridClipper {
    ...
    pub fn clip_with_progress(
        ...
        let tracker =
            progress_tracker.unwrap_or_else(|| Arc::new(ProgressTracker::new("Hybrid Clipping")));

        tracker.start("Hybrid clipping operation", None);
        tracker.set_phase(
            ProgressPhase::Initializing,
            Some("Starting hybrid clipping".to_string()),
        );
        ...
        tracker.set_phase(
            ...
            Some("Validating configuration".to_string()),
        );
    ...
    fn plan_hybrid_strategy(
        &self,
        config: &EngineConfig,
        _plan: &CutPlan,
    ) -> TrimXResult<HybridStrategy> {
        ...
        let start_aligned = self
            .keyframe_analyzer
            .is_keyframe_aligned(&gop_analysis, config.start_time);
        let end_aligned = self
            .keyframe_analyzer
            .is_keyframe_aligned(&gop_analysis, config.end_time);
        ...
        if middle_duration < self.min_copy_duration {
            debug!(
                "Middle segment too short ({:.2}s), using full re-encoding",
                middle_duration
            );
            return Ok(HybridStrategy::FullReencode);
        }
        ...
        info!(
            "Using three-way hybrid: leading {:.2}s, middle {:.2}s, trailing {:.2}s",
    ...
    fn perform_gop_analysis(&self, input_path: &str) -> TrimXResult<GOPAnalysis> {
        info!(
            "Performing GOP analysis for hybrid clipping: {}",
            input_path
        );
    ...
    fn plan_three_way_segments_with_gop_analysis(
        &self,
        config: &EngineConfig,
        gop_analysis: &GOPAnalysis,
    ) -> TrimXResult<ThreeWaySegments> {
        ...
        if self.debug {
            ...
            debug!(
                "  Original range: {:.3}s - {:.3}s",
                config.start_time, config.end_time
            );
            debug!(
                "  Leading segment: {:.3}s - {:.3}s",
                config.start_time, middle_start
            );
            debug!(
                "  Middle segment: {:.3}s - {:.3}s",
                middle_start, middle_end
            );
            debug!(
                "  Trailing segment: {:.3}s - {:.3}s",
                middle_end, config.end_time
            );
        }
    ...
    fn execute_three_way_hybrid(
        &self,
        config: EngineConfig,
        segments: ThreeWaySegments,
    ) -> TrimXResult<ClippingProgress> {
        let temp_dir = std::env::temp_dir().join("trimx_hybrid");
        std::fs::create_dir_all(&temp_dir).map_err(TrimXError::IoError)?;
        ...
        if leading_duration > 0.01 {
            // Only process if segment is meaningful
            info!(
                "Step 1/4: Re-encoding leading segment ({:.3}s to {:.3}s, duration: {:.3}s)",
        ...
        let middle_duration = segments.middle_end - segments.middle_start;
        if middle_duration > 0.01 {
            // Only process if segment is meaningful
            info!(
                "Step 2/4: Stream copying middle segment ({:.3}s to {:.3}s, duration: {:.3}s)",
        ...
        if trailing_duration > 0.01 {
            // Only process if segment is meaningful
            info!(
                "Step 3/4: Re-encoding trailing segment ({:.3}s to {:.3}s, duration: {:.3}s)",
        ...
        info!(
            "Hybrid clipping completed successfully with {} segments",
            segment_files.len()
        );
        Ok(ClippingProgress {
            ...
            description: format!(
                "Hybrid clipping completed with {} segments",
                segment_files.len()
            ),
            eta: None,
        })
    ...
    fn concatenate_segments(&self, input_files: &[String], output_file: &str) -> TrimXResult<()> {
        ...
        if input_files.len() == 1 {
            ...
            std::fs::copy(&input_files[0], output_file).map_err(TrimXError::IoError)?;
            return Ok(());
        }
        ...
        match self.concat_with_demuxer(&concat_list_path, output_file) {
            ...
            Err(e) => {
                ...
                warn!(
                    "Demuxer concatenation failed: {}, trying manual approach",
                    e
                );
    ...
    fn create_concat_list(&self, input_files: &[String]) -> TrimXResult<String> {
        ...
        std::fs::write(&concat_list_path, list_content).map_err(TrimXError::IoError)?;
    ...
    fn concat_with_demuxer(&self, concat_list_path: &str, output_file: &str) -> TrimXResult<()> {
        ...
        let mut output_ctx =
            ffmpeg_next::format::output(output_file).map_err(|e| TrimXError::ClippingError {
    ...
    fn concat_manually(&self, input_files: &[String], output_file: &str) -> TrimXResult<()> {
        ...
        let first_input =
            ffmpeg_next::format::input(&input_files[0]).map_err(|e| TrimXError::ClippingError {
        ...
        for (file_index, input_file) in input_files.iter().enumerate() {
            ...
            let mut input_ctx =
                ffmpeg_next::format::input(input_file).map_err(|e| TrimXError::ClippingError {
    ...
    pub fn estimate_output_size(&self, config: &EngineConfig, plan: &CutPlan) -> TrimXResult<u64> {
        ...
        match strategy {
            HybridStrategy::FullStreamCopy => self.copy_engine.estimate_output_size(config),
            HybridStrategy::FullReencode => self.reencode_engine.estimate_output_size(config),
            HybridStrategy::ThreeWay(segments) => {
                ...
                let reencode_size = self
                    .reencode_engine
                    .estimate_output_size(&reencode_config)?;
